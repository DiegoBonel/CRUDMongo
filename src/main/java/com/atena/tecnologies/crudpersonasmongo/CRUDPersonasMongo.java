/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Project/Maven2/JavaApp/src/main/java/${packagePath}/${mainClassName}.java to edit this template
 */

package com.atena.tecnologies.crudpersonasmongo;


import com.mongodb.MongoClient;
import com.mongodb.MongoClientURI;
import com.mongodb.client.MongoCollection;
import com.mongodb.client.MongoCursor;
import com.mongodb.client.MongoDatabase;

import java.util.ArrayList;
import java.util.Scanner;

import org.bson.Document;
import com.mongodb.MongoClient;
public class CRUDPersonasMongo {

    public static void main(String[] args) {
        
        MongoClientURI uri = crearConexion();
        Scanner teclado = new Scanner(System.in);
        
        try(MongoClient mongoClient = new MongoClient(uri))
        {
            MongoDatabase database = mongoClient.getDatabase("Personas");
            
            // Ingresar personas
//            mostrarColeccion(database, "persona");
//            System.out.println("Nombre de la persona");
//            String nombre = teclado.nextLine();
//            System.out.println("Apellido de la persona");
//            String apellido = teclado.nextLine();
//            System.out.println("Edad de la persona");
//            String edad = teclado.nextLine();
//            insertarPersona(database, "persona", nombre, apellido, edad);
//            mostrarColeccion(database, "persona");

//            BUSCAR POR NOMBRE
//            mostrarColeccion(database, "persona");
//            System.out.println("Que persona esta buscando?");            
//            String buscarPersona = teclado.nextLine();
//            buscarPorNombre(database, "persona", buscarPersona);
            
            //Eliminar persona
//            mostrarColeccion(database, "persona");
//            System.out.println("Persona que se va a eliminar");
//            String eliminarPersona = teclado.nextLine();
//            eliminarPersona(database, "persona", eliminarPersona);
//            mostrarColeccion(database, "persona");
            
            //Actualizar persona
            mostrarColeccion(database, "persona");
            System.out.println("Persona que se va a actualizar");
            String actualizaNombre= teclado.nextLine();
            System.out.println("Ingrese el nuevo apellido");
            String actualizaApellido= teclado.nextLine();
            actualizarPersona(database, "persona", actualizaNombre, actualizaApellido);
            mostrarColeccion(database, "persona");

        } catch (Exception e)
        {   
            System.out.println(e);
        }
    }
    
    
    // METODO PARA CREAR LA CONEXION A MONGODB
    public static MongoClientURI crearConexion() {
        System.out.println("PRUEBA CONEXION MONGODB");
        
        MongoClientURI uri = new MongoClientURI("mongodb+srv://diego:12345@personas.golpbaq.mongodb.net/?retryWrites=true&w=majority");
        
        return uri;
    }
    // MUESTRA TODOS LOS DOCUMENTOS DE LA COLECCION USUARIOS
    public static void mostrarColeccion(MongoDatabase database, String coleccion) {
        MongoCollection<Document> colec = database.getCollection(coleccion);
        
        MongoCursor<Document> document = colec.find().iterator();
        
         while(document.hasNext()){
                ArrayList<Object> veri = new ArrayList(document.next().values());
                for (int i = 0; i < veri.size(); i++)
                {
                System.out.println(veri.get(i));    
                }
            }
    }
    
    // METODO PARA INSERTAR UN DOCUMENTO (REGISTRO)
    public static void insertarPersona(MongoDatabase database, String coleccion, String nombre, String apellido, String edad) {
        MongoCollection<Document> collection = database.getCollection(coleccion);
        
        // CREA EL DOCUMENTO(REGISTRO) E INSERTA LA INFORMACION RECIBIDA
        
        Document documento = new Document();
        documento.put("nombre", nombre);
        documento.put("apellido", apellido);
        documento.put("edad", edad);
        collection.insertOne(documento);
        
    }
               
    // MUESTRA TODOS LOS DOCUMENTOS DE LA COLECCION USUARIOS QUE COINCIDAN CON EL NOMBRE
    public static void buscarPorNombre(MongoDatabase database, String coleccion, String nombre) {
        MongoCollection<Document> collection = database.getCollection(coleccion);
        
        // CREAMOS LA CONSULTA CON EL CAMPO NOMBRE
        Document consulta = new Document();
        consulta.put("nombre", nombre);
        
        // BUSCA Y MUESTRA TODOS LOS DOCUMENTOS QUE COINCIDAN CON LA CONSULTA
        MongoCursor<Document> cursor = collection.find(consulta).iterator();
        while(cursor.hasNext()) {
            System.out.println("-- " + cursor.next().get("edad") + " años");
        }
    }
    
    // METODO PARA ACTUALIZAR UN DOCUMENTO (REGISTRO)
    public static void actualizarPersona(MongoDatabase database, String coleccion, String nombre, String apellido) {
        MongoCollection<Document> collection = database.getCollection(coleccion);
        
        // SENTENCIA CON LA INFORMACION A REMPLAZAR
        Document actualizarPersona = new Document();
        actualizarPersona.append("$set", new Document().append("apellido", apellido));
        
        // BUSCA EL DOCUMENTO EN LA COLECCION
        Document buscarPorNombre = new Document();
        buscarPorNombre.append("nombre", nombre);
        
        // REALIZA EL UPDATE
        collection.updateOne(buscarPorNombre, actualizarPersona);
    }
    
    // METODO PARA ELIMINAR UN DOCUMENTO (REGISTRO)
    public static void eliminarPersona(MongoDatabase database, String coleccion, String nombre) {
        MongoCollection<Document> collection = database.getCollection(coleccion);
        
        collection.deleteOne(new Document().append("nombre", nombre));
    }
}
